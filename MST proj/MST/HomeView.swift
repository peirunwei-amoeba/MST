    ...
    private var assignmentSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if upcomingAssignments.isEmpty {
                ...
                VStack(spacing: 0) {
                    ForEach(Array(upcomingAssignments.prefix(4).enumerated()), id: \.element.id) { index, assignment in
                        ConcentricAssignmentRow(assignment: assignment) {
                        ...
                        if index < min(upcomingAssignments.count - 1, 3) {
                            Divider()
                                .padding(.leading, 52)
                        }
                    }
                ...
                if upcomingAssignments.count > 4 {
                    Button {
                        showingAllAssignments = true
                    } label: {
                        HStack {
                            Text("+ \(upcomingAssignments.count - 4) more")
                                .font(.subheadline.weight(.medium))
...
struct ConcentricAssignmentRow: View {
    ...
    var body: some View {
        Button {
            ...
            HStack(spacing: 14) {
                // Completion button with concentric circle design
                Button {
                    onToggleComplete()
                } label: {
                    ZStack {
                        Circle()
                            .fill(assignment.isCompleted ? Color.green.opacity(0.15) : Color.secondary.opacity(0.1))
                            .frame(width: 36, height: 36)

                        Circle()
                            .strokeBorder(assignment.isCompleted ? Color.green : Color.secondary.opacity(0.3), lineWidth: 2)
                            .frame(width: 36, height: 36)
